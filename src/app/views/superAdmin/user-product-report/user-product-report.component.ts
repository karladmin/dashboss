...
@Component({
  selector: 'app-user-product-report',
  templateUrl: './user-product-report.component.html',
  styleUrls: ['./user-product-report.component.scss'],
})
export class UserProductReportComponent implements OnInit {
  start_date: any;
  end_date: any;
  products$: any;
  data: any;
  ...
  saveOwner(e?) {
    ...
    this.api.postDataWithToken('report/user-detail', data).subscribe(
      (success: any) => {
        if (success.success) {
          this.data = success.data.appointment;
          this.products = success.data.product;
          this.data.forEach((element) => {
            element.userName = element.master.app_customer.name;
            element.branchName = element.master.branch.name;
            element.serviceName = element.service.name;
            element.staffName = element.staff.name;
            element.useImage = element.master.app_customer.imageUri;
          });
          if (e) {
            this.loading = false;
          }
  ...
  exportCSV() {
    if (this.activeTab == 0) {
      ...
      this.api.exportCsvFile(csv, 'User Appointment Report');
    } else {
      ...
      this.api.exportCsvFile(csv, 'User Product Report');
    }
  }
  exportPDF() {
    if (this.activeTab == 0) {
      ...
      this.api.exportPdfFile(html, 'User Appointment Report');
    } else {
      const pdfTable = this.content2.nativeElement;
      var html = htmlToPdfmake(pdfTable.innerHTML);
      this.api.exportPdfFile(html, 'User Product Report');
    }
  }
}
